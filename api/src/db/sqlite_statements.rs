...
pub const SELECT_ANIME: &str =
    "SELECT proxer_id, proxer_name, mal_id, episode_count
    FROM anime
    WHERE proxer_id = (:proxer_id);";
...
pub const SELECT_ALL_OST: &str = 
    "SELECT proxer_id, ost_type, number, name, artist, video_url
    FROM ost;";

pub const SELECT_OST: &str = 
    "SELECT proxer_id, ost_type, number, name, artist, video_url
    FROM ost
    WHERE proxer_id = (:proxer_id)
    AND ost_type = (:ost_type)
    AND number = (:number);";
...
pub const INSERT_USER_ANIME: &str =
    "INSERT OR IGNORE INTO user_anime (user_id, episode_progress, proxer_id, date_added)
    VALUES (:user_id, :episode_progress, :proxer_id, :date_added);";

pub const INSERT_ANIME: &str =
    "INSERT OR IGNORE INTO anime (proxer_id, proxer_name, mal_id, episode_count)
    VALUES (:proxer_id, :proxer_name, :mal_id, :episode_count);";

pub const INSERT_OST: &str =
    "INSERT OR IGNORE INTO ost (proxer_id, ost_type, number, name, artist, video_url)
    VALUES (:proxer_id, :ost_type, :number, :name, :artist, :video_url)";

// ##########################################################################################################
// UPDATE
// ##########################################################################################################

pub const UPDATE_OST: &str =
    "UPDATE ost
    SET name = (:name), artist = (:artist), video_url = (:video_url)
    WHERE proxer_id = (:proxer_id)
    AND ost_type = (:ost_type)
    AND number = (:number);";
